...
def binary_pascal_mirror(n, vertical_reflections=1, horizontal_reflections=1):
    triangle = [[1]]
    for i in range(1, n):
        row = [1]
        last_row = triangle[-1]
    ...
    mirrored_triangle = binary_triangle

    # Perform the mirroring operations the specified number of times
    for _ in range(vertical_reflections):
        mirrored_triangle = mirrored_triangle + mirrored_triangle[::-1]
    for _ in range(horizontal_reflections):
        mirrored_triangle = [row + row[::-1] for row in mirrored_triangle]

    mirrored_triangle = np.array([[int(b) for b in row] for row in mirrored_triangle])
...
initial_state = array_to_bin_matrix(binary_pascal_mirror(34))
...
ani = animation.FuncAnimation(fig, animate, frames=81, interval=200)

# Save the animation as a GIF
#ani.save('Pascal Reflective Aggregate 11,5,3.gif', writer=PillowWriter(fps=1))
